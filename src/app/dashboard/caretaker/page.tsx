...
// ✅ FIX: Define a minimal, explicit type for the user object to avoid using 'any'
type AuthUser = {
  full_name?: string;
  [key: string]: unknown;
};
...
export default function CaretakerDashboard() {
  ...
  const [activeFilter, setActiveFilter] = useState<"all" | "maintenance" | "inspections">("all");
  ...
  const filteredTasks = tasks.filter((t) => {
    ...
    return t.type === activeFilter || (activeFilter === "inspections" && t.type === "inspection");
  });

  const stats = {
    emergency: tasks.filter((t) => t.priority === "emergency" && t.status !== "completed").length,
    pending: tasks.filter((t) => t.status === "assigned" || t.status === "in_progress").length,
    completed: tasks.filter((t) => t.status === "completed").length,
  };

  // ✅ FIX: Replaced 'any' with a safe, explicit type assertion
  const userName = (user as AuthUser | null | undefined)?.full_name || "Caretaker";
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 max-w-md mx-auto relative shadow-2xl sm:border-x sm:border-slate-200">
      ...
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white border-t border-slate-200 px-6 py-3 flex justify-around items-center z-30 shadow-lg">
        <NavButton icon="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" label="Home" active />
        <NavButton icon="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" label="Tasks" />
        <NavButton icon="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" label="Map" />
        <NavButton icon="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" label="Profile" />
      </nav>
    </div>
  );
}
...
function StatBox({ label, value, color, bg, pulse }: { label: string; value: number | string; color: string; bg: string; pulse?: boolean }) {
  return (
