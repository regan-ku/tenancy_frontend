...
import LandlordApplicationReviewModal from "@/components/landlord/LandlordApplicationReviewModal";
import TerminationSettlementModal from "@/components/applications/TerminationSettlementModal"; // Shared component
...
// ✅ Helper to map transfers to LandlordApplication format for the "Awaiting Payment" tab
const mapTransferToApp = (transfer: any): LandlordApplication => ({
  id: transfer.id,
...
// ✅ Helper to map terminations to LandlordApplication format
const mapTerminationToApp = (notice: any): LandlordApplication => ({
  id: notice.id,
...
export default function LandlordOperationsPage() {
  ...
  const [transfers, setTransfers] = useState<LandlordTransfer[]>([]);
  const [terminations, setTerminations] = useState<LandlordTermination[]>([]);
  const [loading, setLoading] = useState(true);

  const [reviewingApp, setReviewingApp] = useState<LandlordApplication | null>(
    null,
  );
  ...
  // Waiver Modal State
  const [waiverApp, setWaiverApp] = useState<LandlordApplication | null>(null);
  ...
  const [undoWaiverApp, setUndoWaiverApp] =
  ...
  const refreshData = async () => {
    ...
    const [apps, trans, terms] = await Promise.all([
      landlordOperationsApi.getApplications(),
      landlordOperationsApi.getTransfers(),
      landlordOperationsApi.getTerminations(),
    ]);
  ...
  const awaitingPaymentApps: LandlordApplication[] = [
  ...
  const handleSubmitWaivers = async () => {
    ...
    try {
      const res = await landlordOperationsApi.applyWaiver(
        waiverApp.id,
        selectedWaivers,
        waiverReason || "Waived by landlord",
      );
      alert(`✅ ${res.data.detail || "Waivers applied successfully."}`);
      setWaiverApp(null);
      setSelectedWaivers([]);
      setWaiverReason("");
      refreshData();
  ...
  const handleUndoWaivers = async () => {
    ...
    try {
      const res = await landlordOperationsApi.revokeWaiver(
        undoWaiverApp.id,
        selectedUndoWaivers,
        undoWaiverReason || "Waiver revoked by landlord",
      );
      alert(`✅ ${res.data.detail || "Waivers revoked successfully."}`);
  ...
  const handleManagerCancel = async (appId: number) => {
    ...
    try {
      await landlordOperationsApi.managerCancelApplication(appId, reason);
  ...
  return (
    <div className="space-y-8">
      ...
      {reviewingApp && (
        <LandlordApplicationReviewModal
