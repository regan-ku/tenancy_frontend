...
export default function LandlordPropertyDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedData = await landlordPropertiesApi.getPropertyDetail(
        Number(id),
      );
  ...
  return (
    <div className="space-y-6">
      {/* Ownership / Delegation Banner */}
      <div
        className={`p-4 rounded-xl border flex items-center gap-3 ${!property.is_delegated ? "bg-emerald-50 border-emerald-200" : "bg-indigo-50 border-indigo-200"}`}
      >
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${!property.is_delegated ? "bg-emerald-500" : "bg-indigo-500"}`}
        >
          {!property.is_delegated ? "🏠" : "🤝"}
        </div>
        <div className="flex-1">
          <p className="text-sm font-bold text-slate-800">
            {!property.is_delegated
              ? "Self-Managed Property"
              : `Delegated to ${property.agency_name || "Agency"}`}
          </p>
          <p className="text-xs text-slate-500">
            {!property.is_delegated
              ? "You have full administrative control over this property."
              : `Scope: ${property.delegation_type?.replace("_", " ").toUpperCase() || "MANAGEMENT"} | The agency handles day-to-day operations.`}
          </p>
        </div>
      </div>
...
function PropertyTeamTab({ propertyId }: { propertyId: number }) {
  ...
  useEffect(() => {
    landlordPropertiesApi.getPropertyTeam(propertyId).then((data) => {
      setTeam(data);
      setLoading(false);
    });
  }, [propertyId]);
...
function PropertyMediaHub({
  ...
  const loadData = async () => {
    ...
    try {
      const [groupsData, mediaData] = await Promise.all([
        landlordUnitManagementApi.getUnitGroups(propertyId),
  ...
  const handlePropertyCoverChange = async (
    ...
    try {
      const newUrl = await landlordPropertiesApi.updateCoverPhoto(
        propertyId,
        e.target.files[0],
      );
      onPropertyCoverChange(newUrl);
  ...
  const handlePropertyMediaUpload = async (
    ...
    try {
      for (const file of files) {
        ...
        await landlordPropertiesApi.uploadPropertyMedia(
          propertyId,
          file,
          mediaType,
        );
  ...
  const handleGroupCoverChange = async (
    ...
    try {
      await landlordUnitManagementApi.updateUnitGroup(propertyId, groupId, fd);
      const groupsData =
        await landlordUnitManagementApi.getUnitGroups(propertyId);
      setUnitGroups(groupsData);
  ...
  const handleGroupMediaUpload = async (
    ...
    try {
      for (const file of files) {
        const mediaType = file.type.startsWith("video") ? "video" : "image";
        await landlordUnitManagementApi.uploadMedia(
          propertyId,
          file,
  ...
  const handleDeleteMedia = async (mediaId: number) => {
    ...
    try {
      await landlordUnitManagementApi.deleteMedia(propertyId, mediaId);
  ...
  const handleUpdateCaption = async (mediaId: number, newCaption: string) => {
    try {
      await landlordPropertiesApi.updateMediaCaption(
        propertyId,
        mediaId,
        newCaption,
      );
  ...
  const handleSetAsCover = async (mediaUrl: string) => {
    ...
    try {
      const newUrl = await landlordPropertiesApi.setCoverFromMedia(
        propertyId,
        mediaUrl,
      );
      onPropertyCoverChange(newUrl);
