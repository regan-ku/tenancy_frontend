...
import {
  landlordUnitManagementApi, // ✅ UPDATED: Swapped to landlord API
  UnitGroup,
  Unit,
} from "@/api/landlordUnitManagement.api"; // ✅ UPDATED: Swapped to landlord API file
import apiClient from "@/api/axios";
import { endpoints } from "@/config/endpoints";
import AddTenantModal from "../agency/AddTenantModal"; // Ensure this points to your landlord components folder
...
export default function UnitManagementTab({
  ...
  const loadData = async () => {
    ...
    try {
      const [groupsData, unitsData] = await Promise.all([
        landlordUnitManagementApi.getUnitGroups(propertyId), // ✅ UPDATED
        landlordUnitManagementApi.getUnits(propertyId), // ✅ UPDATED
      ]);
  ...
  const handleUpdateGroup = async (
    ...
    try {
      await landlordUnitManagementApi.updateUnitGroup(
        // ✅ UPDATED
        propertyId,
        groupId,
        updates,
  ...
  const handleUpdateUnit = async (unitId: number, updates: Partial<Unit>) => {
    try {
      await landlordUnitManagementApi.updateUnit(propertyId, unitId, updates); // ✅ UPDATED
      await loadData();
      setEditingUnit(null);
      alert("✅ Unit updated!");
    } catch (error) {
  ...
  const handleAddUnit = async (groupId: number, floorNumber: number) => {
    try {
      await landlordUnitManagementApi.addUnitToGroup(
        // ✅ UPDATED
        propertyId,
        groupId,
        floorNumber,
  ...
  const handleDeleteUnit = async (unitId: number, unitCode: string) => {
    ...
    try {
      await landlordUnitManagementApi.deleteUnit(propertyId, unitId); // ✅ UPDATED
      await loadData();
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        {!hasAnyData ? (
          ...
          // Iterate over groupsToRender instead of just the units
