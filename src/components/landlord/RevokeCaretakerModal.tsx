...
interface RevokeCaretakerModalProps {
  caretaker: LandlordStaffMember;
  onClose: () => void;
  onSuccess: () => void;
}

export default function RevokeCaretakerModal({
  caretaker,
  onClose,
  onSuccess,
}: RevokeCaretakerModalProps) {
  ...
  const handleRevoke = async () => {
    ...
    try {
      await landlordStaffApi.deactivateCaretaker(caretaker.id, reason);
      onSuccess();
    } catch (err: any) {
      console.error("Failed to revoke caretaker", err);
      const errMsg =
        err.response?.data?.detail ||
        err.response?.data?.error ||
        "Failed to revoke caretaker account.";
      setError(errMsg);
    } finally {
      setIsRevoking(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        <div className="p-6 border-b border-slate-100 bg-red-50">
          ...
          <p className="text-sm text-red-600 mt-1">
            This will permanently suspend <strong>{caretaker.full_name}</strong>{" "}
            from accessing the system. They will be immediately logged out and
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., Caretaker resigned, Contract expired, Security breach..."
            />
          </div>
