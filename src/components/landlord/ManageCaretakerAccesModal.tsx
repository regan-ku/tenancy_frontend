...
interface ManageCaretakerAccessModalProps {
  caretaker: LandlordStaffMember;
  onClose: () => void;
  onRevoke: () => void; // Triggers the full account revocation modal
  onAssignmentChange: () => void; // Refreshes parent data
}

export default function ManageCaretakerAccessModal({
  caretaker,
  onClose,
  onRevoke,
  onAssignmentChange,
}: ManageCaretakerAccessModalProps) {
  ...
  const fetchAssignments = async () => {
    setLoading(true);
    try {
      const data = await landlordStaffApi.getCaretakerAssignments(caretaker.id);
      setAssignments(data);
    } catch (err) {
  ...
  const handleRemoveAccess = async (
    ...
    if (
      !confirm(`Remove ${caretaker.full_name}'s access to "${propertyName}"?`)
    )
      return;
    ...
    setError("");
    try {
      await landlordStaffApi.removeCaretakerFromProperty(
        caretaker.id,
        propertyId,
      );
      setAssignments((prev) =>
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-primary-dark">
              Manage Access: {caretaker.full_name}
            </h2>
            <p className="text-sm text-slate-500 mt-1">
              {caretaker.email} • {caretaker.phone_number}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 space-y-2">
              ...
              <p className="text-slate-500">
                This caretaker is not assigned to any properties yet.
              </p>
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500">
                        Assigned by: {assignment.assigned_by_agency || "You"} •
                        Since:{" "}
