...
import { TenantFinancialInfo } from "@/api/landlordUnitManagement.api"; // ✅ UPDATED IMPORT
import { tenanciesApi } from "@/api/tenancies.api";
import { landlordUnitManagementApi } from "@/api/landlordUnitManagement.api"; // ✅ UPDATED IMPORT
import { landlordPropertiesApi } from "@/api/landlordProperties.api"; // ✅ UPDATED IMPORT
...
export default function TransferTenantModal({
  ...
  const loadProperties = async () => {
    ...
    try {
      // ✅ UPDATED: Uses Landlord Properties API
      const props = await landlordPropertiesApi.getProperties();
      setProperties(
        props.map((p) => ({
          id: p.id,
          name: p.name,
  ...
  const loadUnitsForProperty = async (propId: number) => {
    ...
    try {
      // ✅ UPDATED: Uses Landlord Unit Management API
      const units = await landlordUnitManagementApi.getUnits(propId);
