...
export default function MarketplaceNavbar() {
  ...
  // ✅ SMART BUTTON LOGIC:
  // - Unauthenticated/Tenants -> "List Property" -> /register (Backend handles routing after signup)
  // - Authenticated Managers -> "Add Property" -> /properties/wizard
  const propertyButtonText = isAuthenticated && isManager ? "Add Property" : "List Property";
  const propertyButtonHref = isAuthenticated && isManager ? "/properties/wizard" : "/register";
  const propertyButtonTooltip = isAuthenticated && isManager 
    ? "Add a new property to your portfolio" 
    : "Sign up as a landlord or agency to add property";
          ...
          <div className="hidden md:flex md:items-center md:space-x-3">
            {!hasMounted ? (
              ...
              <div className="flex items-center space-x-3">
                <span className="h-9 w-24 bg-slate-100 rounded-lg animate-pulse"></span>
                <span className="h-9 w-20 bg-slate-100 rounded-lg animate-pulse"></span>
              </div>
            ) : (
              <>
                {/* ✅ ALWAYS VISIBLE "LIST/ADD PROPERTY" BUTTON */}
                <Link 
                  href={propertyButtonHref}
                  title={propertyButtonTooltip}
                  className="flex items-center gap-2 text-primary border border-primary/30 hover:bg-primary/5 px-4 py-2 rounded-lg text-sm font-semibold transition-all"
                >
                ...
                {!isAuthenticated ? (
                  <>
                    <Link href="/login" className="text-sm font-semibold text-slate-600 hover:text-primary transition-colors px-3 py-2">
                      Sign In
                    </Link>
                  ...
                  <>
                    ...
                    <Link 
                      href="/dashboard" 
                      className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-4 py-2.5 rounded-lg text-sm font-semibold shadow-sm transition-all"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="px-4 pt-2 pb-4 space-y-1">
            ...
            <div className="pt-4 mt-4 border-t border-slate-200 space-y-3">
              {!hasMounted ? (
                ...
                <>
                  {/* ✅ ALWAYS VISIBLE MOBILE "LIST/ADD PROPERTY" BUTTON */}
                  <Link 
                    href={propertyButtonHref} 
                    onClick={() => setIsMobileMenuOpen(false)} 
                    title={propertyButtonTooltip}
                    className="flex items-center justify-center gap-2 w-full px-3 py-3 rounded-lg text-base font-semibold text-primary border border-primary/30 bg-primary/5 hover:bg-primary/10 transition-all"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                    </svg>
                  ...
                  {!isAuthenticated ? (
                    ...
                    <>
                      <Link href="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center gap-2 w-full px-3 py-3 rounded-lg text-base font-semibold text-white bg-slate-900 hover:bg-slate-800 shadow-sm transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                        </svg>
                        Go to Dashboard
