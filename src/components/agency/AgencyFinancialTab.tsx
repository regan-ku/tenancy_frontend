...
import TerminateTenancyModal from "./TerminateTennacyModal";
import TransferTenantModal from "./TransferTenantModal";
import ExtendTenancyModal from "./ExtendTenancyModal";
...
export default function TenantsFinancialsTab({
  ...
  const [editingApplication, setEditingApplication] =
    useState<AgencyApplication | null>(null);
  ...
  const loadTenants = async () => {
    setLoading(true);
    try {
      // ✅ FIX: Cast to 'any' to handle both array and paginated responses
      const response =
        await agencyUnitManagementApi.getTenantFinancials(propertyId);
      const data = response as any;

      // ✅ FIX: Handle both direct array and paginated { results: [] } responses
  ...
  const loadTenantApplications = async (tenantId: number) => {
    try {
      const apps = await agencyOperationsApi.getTenantApplications(tenantId);
  ...
  const handleEditApplication = (app: AgencyApplication) => {
...
function TenantDetailModal({
  ...
  tenantApplications: AgencyApplication[];
  onClose: () => void;
  onTerminate: () => void;
  onTransfer: () => void;
  onExtend: () => void;
  onNotes: () => void;
  onEditApplication: (app: AgencyApplication) => void;
